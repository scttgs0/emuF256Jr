...
namespace FoenixCore.Simulator.Devices
{
    ...
    public class GabeSDController : SDCardDevice
    {
            ...
            if (isPresent)
            {
                ...
                if (IsoMode)
                {
                    ...
                    if (firstSector[0] != 0xeb && firstSector[0x1FE] == 0x55 && firstSector[0x1FF] == 0xAA && (firstSector[0x1BE] & 0x80) != 0 )
                    {
                        ...
                        {
                            case 1:
                                FileSystemType = FSType.FAT12;
                                break;

                            case 4:
                                FileSystemType = FSType.FAT16; // FAT16 with less than 65536 sectors (32MB)
                                break;

                            case 0xc:
                                FileSystemType = FSType.FAT32;
                                break;

                            case 0xE:
                                FileSystemType = FSType.FAT16; // FAT16B with LBA
                                break;

                            case 0x16:
                                FileSystemType = FSType.FAT16; //hidden FAT16B
                                break;
                        }
                ...
                if (!IsoMode)
                {
                    ...
                    sectors_per_cluster = (byte)(ClusterSize / 512);
                    ...
                    int capacity = Capacity * 1024 * 1024 - (1+1+ reserved_sectors) * 512 - BOOT_SECTOR_ADDR - root_sectors * 512;  // remove the reserved and offset spaces, boot sector, mbr and root area
                    int sector_count = 2 + BOOT_SECTOR_ADDR / 512 + reserved_sectors + 32;

                    int req_cluster = capacity / ClusterSize;
                    ...
                    {
                        case FSType.FAT12:
                            sector_count += sectors_per_fat * 1024 / 3 * ClusterSize / 512;
                            break;

                        case FSType.FAT16:
                            sector_count += sectors_per_fat * 256 * ClusterSize / 512;
                            break;

                        case FSType.FAT32:
                            sector_count += sectors_per_fat * 128 * ClusterSize / 512;
                            break;
                    }
            ...
            if (page == 0)
            {
                switch (FileSystemType)
                {
                    ...
                    case FSType.FAT32:
                        ...
                        for (int i = 0; i < 32 * 512 / ClusterSize - 1; ++i)
                        {
        ...
        private void CreateEmptyFile(int size, int page)
        {
            ...
            string filename = SDCardPath + "\\" + randomFileName + ".new";
        ...
        private byte[] GetData_ISO(int page)
        {
            ...
            {
                byte[] buffer = new byte[512];
                FileStream stream = new(SDCardPath, FileMode.Open, FileAccess.Read);
        ...
        private void SetData_ISO(int page, byte[] buffer)
        {
            if ((page >= 0) && (page < 512 * 4096)) // test if we are with in 256MB
            {
                FileStream stream = new(SDCardPath, FileMode.Open, FileAccess.Write);
