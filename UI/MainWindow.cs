...
#pragma warning restore CS0649
        ...
        public void SDCardInterrupt(CH376SInterrupt irq)
        {
            ...
            byte mask = kernel.MemMgr.ReadByte(MemoryMap.IRQ_CTRL.MASK+1);
            if (!kernel.CoreCpu.DebugPause && (~mask & (byte)Register1.INT07_SDCARD) == (byte)Register1.INT07_SDCARD)
            {
                // Set the SD Card Interrupt
                byte IRQ1 = kernel.MemMgr.ReadByte(MemoryMap.IRQ_CTRL.PENDING+1);
        ...
        */
        ...
        private void on_Gpu_SOF()
        {
            ...
            byte mask = kernel.MemMgr.ReadByte(MemoryMap.IRQ_CTRL.MASK);
            if (!kernel.CoreCpu.DebugPause)
            {
                // Set the SOF Interrupt
                byte IRQ0 = kernel.MemMgr.ReadByte(MemoryMap.IRQ_CTRL.PENDING);
